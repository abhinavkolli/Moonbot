...
class UIInfo(QtGui.QWidget):
    ...
    def __init__(self):
        ...
        self.setWindowTitle('Moonbot Information')
        self.setStyleSheet("background-color:#203044;")
        self.setWindowIcon(QtGui.QIcon("MoonbotIcon.png"))
    ...
    def BuildWindowLayout(self):
        ...
        # Section 1 ==================================================================================
        self.lblTitleSection1 = QtGui.QLabel("How does Moonbot trading strategy work?")
        ...
        self.lblTxtSection11.setText("<p style=\"margin-left: 45px\"><b></b>When running in live trading mode, Moonbot updates the price chart every 10 seconds with the most recent middle-market price from Coinbase Pro exchange. It also computes a MACD-like indicator (bottom yellow graph) that helps finding buy and sell opportunities. Moonbot strategy is simple:<br/><br/><b>1. Wait the dip.</b> First, Moonbot is waiting for a buy opportunity. Ideally buy oppotunities are detected at the end of a dip.<br/><br/><b>2. Buy the dip. </b> If a buy opportunity is detected AND if the current price is below the red dashed line (the « risk line »), Moonbot sends a market buy order to Coinbase Pro in order to buy the crypto asset. The amount of fiat money that is invested can be adjusted in the Settings.<br/><br/><b>3. Wait the top. </b> Moonbot waits for the next sell oppotunity that would generate a positive profit, in other words which will at least cover the 2 market order fees (buy + sell fees).<br/><br/><b>4. Sell the top. </b> If a sell oppotunity meets the conditions explained at step 3, the entirety of your crypto asset balance is sold into fiat, and you funds should have increased. Then Moonbot goes back to step 1 for another trade<br/>")
        ...
        self.lblTxtSection12.setText("<p style=\"margin-left: 45px\"><b></b>In simulation mode, Trading strategy is the same as in Live trading mode, excepted that it is performed on historic samples that quickly scroll allowing you to test different tunnings or trading pairs. No orders are sent to Coinbase Pro in simulation mode, trades are entirely simulated. <br/>Simulation mode will familiarise you with Moonbot trading strategy and how to tune it. <br/></p>")
        ...
        self.lblTxtSection25.setText("Approximate positions of the buy (green symbol) and sell (red symbol) transactions performed by Moonbot.")
        ...
        self.lblTxtSection26.setText("<b></b>Decision indicator chart. It is computed with the subtraction of the blue and orange smoothed prices. It is similar to the well known MACD indicator. If it goes from negative to positive, Moonbot will interpret it as a buy signal. A positive to negative change is identified as a sell signal. The influence of the sensitivity setting is directly visible on this graph as if you increase the sensitivity, more buy and sell signals will appear.")
