...
class GDAXControler(cbpro.OrderBook):
    ...
    def PerformConnexionInitializationAttempt(self):
        ...
        try:
            self.clientAuth = cbpro.AuthenticatedClient(self.api_key, self.api_secret, self.api_passphrase,
        ...
        if (bInternetLinkIsOK == True):
            ...
            # Display error message
            self.theUIGraph.UIGR_updateInfoText(
                "Connexion to Coinbase Pro server failed. Check your internet connexion.", True)
    ...
    def on_close(self):
        ...
        if (self.isRunning == True):  # If we are not exiting app
            if (
                    self.IsConnectedAndOperational != "Requested" and self.IsConnectedAndOperational != "Ongoing"):  # If we are not re-initializing connexion (like settings apply)
